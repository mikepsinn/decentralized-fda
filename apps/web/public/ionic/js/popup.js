...
var inboxButtonClicked = function(){
    ...
    if(typeof OverApps !== "undefined"){
		...
		if(!qm.chrome.windowParams){
			var newTab = window.open('/app/public/#/app/reminders-inbox', '_blank');
			newTab.focus();
			//window.close();
		} else {
...
function hidePopupPostNotificationsDeleteLocalAndClosePopup(){
    ...
    if(qm.notificationsSyncQueue){
		question().innerHTML = "Saving...";
	    const buttonsSection = getButtonsSection()
	    if(buttonsSection){
...
function setInboxButtonListener() {
	let inboxButton = document.getElementById('buttonInbox');
	if (inboxButton) {
		inboxButton.onclick = inboxButtonClicked;
	} else {
		console.error("buttonInbox element not found");
	}
	let inboxImage = document.getElementById('inboxImage');
	if (inboxImage) {
		inboxImage.onclick = inboxButtonClicked;
	} else {
		console.error("inboxImage element not found");
	}
}
function hidePopup(){
...
function unHidePopup(){
	showQuestionBox();
	// Select the div.questionBox element
	var questionBox = getQuestionBox();

	// Get the height and width of the selected element
	var ratingPopupHeight = questionBox.offsetHeight;
	var ratingPopupWidth = questionBox.offsetWidth;

	// Get the computed style of the questionBox element
	var computedStyle = window.getComputedStyle(questionBox);
	
	// Get the bottom margin value, remove 'px' from the end and convert to a number
	var marginBottom = parseFloat(computedStyle.marginBottom);
	
	// Add the bottom margin to the height
	ratingPopupHeight += marginBottom;

// Continue with your existing code...
	ratingPopupHeight += 50;
	ratingPopupWidth += 20;
	// ratingPopupHeight = 220;
	// ratingPopupWidth = 450;
	// Get the screen's width and height
	var screenWidth = window.screen.availWidth;
	var screenHeight = window.screen.availHeight;

	// Calculate the new position for the window
	var newWindowPosX = screenWidth - ratingPopupWidth;
	var newWindowPosY = screenHeight - ratingPopupHeight;
	window.resizeTo(ratingPopupWidth, ratingPopupHeight);

	// Move the window to the new position
	window.moveTo(newWindowPosX, newWindowPosY);
...
function updateQuestion(variableName){
	...
	// if(qmPopup.trackingReminderNotification.longQuestion){
	// Can't use longQuestion because it's for speech recognition and asks for a number when there are face buttons
	// 	questionText = qmPopup.trackingReminderNotification.longQuestion;
	// }
    window.qmLog.pushDebug('popup: Updating question to ' + questionText);
    question().innerHTML = questionText;
    document.title = questionText;
	// Do after 100ms to allow for the question to be set
	setTimeout(unHidePopup, 5000);
    function showLastValuesSection() {
        function setLastValueButtonProperties(textElement, buttonElement, notificationAction) {
            if (notificationAction.modifiedValue !== null) {
	            ...
	            buttonElement.title = notificationAction.longTitle;
            }
            else {
                buttonElement.style.display = "none";
            }
...
function getButtonsSection(){
	return document.getElementById("buttons-section");
}
function getQuestionBox(){
	return document.querySelector("body > div.questionBox")
}
