...
var qm = {
    ...
    api: {
        ...
        post: function(path, body, successHandler, errorHandler){
			//var url = qm.api.getRequestUrl(path, body);  Why add body to url?
            ...
            xhr.onreadystatechange = function(){
                ...
                if(xhr.readyState === 4){
                    ...
                    if ( xhr.status === 200 ) {
						...
						if(xhr.responseText.indexOf("token expired") !== -1){
							qm.auth.setAfterLoginGoToUrlAndSendToLogin(xhr.responseText)
						}
                        if(errorHandler){errorHandler(err);}
                    }
                }
...
// delete items in appSettings.appDesign.wordAliases array that are null
    ...
    notifications: {
        ...
        getMostRecentUniqueNotificationNotInSyncQueue: function(){
	        ...
	        var uniqueNotifications = qm.storage.getItem(qm.items.trackingReminderNotifications);
            if(!uniqueNotifications || !uniqueNotifications.length){
                qmLog.info("No uniqueNotifications in storage");
                return null;
            }
    ...
    speech: {
        ...
        deepThought: function(callback){
			...
			if(!deepThoughts){
				qmLog.warn("No deepThoughts found in qm.staticData.deepThoughts");
				return;
			}
    ...
    urlHelper: {
	    ...
	    prefixQMAPIOriginIfNecessary: function(pathOrUrl){
		    if(pathOrUrl.indexOf('http') === 0){
                pathOrUrl = pathOrUrl.replace('/api/', '/api/dfda/')
                pathOrUrl = pathOrUrl.replace('/v1/', '/')
                pathOrUrl = pathOrUrl.replace('/v2/', '/')
                pathOrUrl = pathOrUrl.replace('/v3/', '/')
                pathOrUrl = pathOrUrl.replace('/v4/', '/')
                 return pathOrUrl
		    }
        ...
        // remove api and version from the path
        pathOrUrl = pathOrUrl.replace('/api/', '/')
        pathOrUrl = pathOrUrl.replace('/v1/', '/')
        pathOrUrl = pathOrUrl.replace('/v2/', '/')
        pathOrUrl = pathOrUrl.replace('/v3/', '/')
        pathOrUrl = pathOrUrl.replace('/v4/', '/')
